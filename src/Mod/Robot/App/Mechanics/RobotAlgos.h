...
inline KDL::JntArray toJntArray(const std::vector<double>& t_JntVals){
    KDL::JntArray t_Array(t_JntVals.size());
    for(int i = 0; i<t_JntVals.size(); i++){
        t_Array.data(i) = t_JntVals[i];
    }
    return t_Array;
}

inline const std::vector<double> toVector(const KDL::JntArray& t_JntArray){
    std::vector<double> t_Vec;
    for(int i = 0; i<t_JntArray.rows(); i++){
        t_Vec.push_back(t_JntArray.data(i));
    }
    return t_Vec;
}
