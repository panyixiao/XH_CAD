...
void RobotProgram::insertCMD_NewMOVE(const std::size_t t_PoseID,
                                     ...
                                     const float t_VBL)
{
    m_commands.push_back(std::make_shared<Robot::MoveCommand>(Robot::MoveCommand(t_Type,
                                                                                t_Prec,
                                                                                t_PoseID,
                                                                                t_V,t_BL,t_VBL)));
}

void RobotProgram::insertCMD_SwitchTool(const ToolType &t_Type,
                                        const uint coordID)
{
    m_commands.push_back(std::make_shared<Robot::ToolCommand>(Robot::ToolCommand(t_Type,
                                                                                coordID)));
}

void RobotProgram::insertCMD_SetToolOn(const Robot::ToolType& t_Type)
{
    m_commands.push_back(std::make_shared<Robot::ToolCommand>(Robot::ToolCommand(t_Type,
                                                                                true)));
}

void RobotProgram::insertCMD_SetToolOff(const ToolType &t_Type)
{
    m_commands.push_back(std::make_shared<Robot::ToolCommand>(Robot::ToolCommand(t_Type,
                                                                                false)));
}

void RobotProgram::insertCMD_ChgCord(const CordType &t_Type, const uint cordID)
{
    m_commands.push_back(std::make_shared<Robot::CoordCommand>(Robot::CoordCommand(t_Type,
                                                                                  cordID)));
}

const std::size_t RobotProgram::addNewPose(const TargetPoint &t_Pnt)
{
    auto pntID = findWaypoint(t_Pnt.getPointHashID());
    if(pntID>0)
        return pntID;
    auto tmp = std::make_shared<TargetPoint>(t_Pnt);
...
{
    std::size_t result = 0;
    auto iter = std::find_if(m_waypoints.cbegin(),m_waypoints.cend(),[&](const TargetPoint_sptr& c_iter){
...
void RobotProgram::Restore(XMLReader &reader)
{
    ...
    for (int i = 0; i < poseNum; i++) {
        auto t_waypoint = std::make_shared<TargetPoint>();
...
void RobotProgram::joinPoseData(const std::vector<TargetPoint_sptr> &t_PoseData)
{
...
void RobotProgram::setWPntPoseAdjust(const int pntID,
                                        const uint rbtID,
                                        const Base::Placement &new_Adjust)
{
    auto& t_PntIter = m_waypoints.at(pntID);
//    if(rbtID == 1)
...
const TargetPoint_sptr RobotProgram::getWaypoint_byID(const size_t wp_id) const
{
...
{
    auto iter = std::find_if(m_waypoints.cbegin(),m_waypoints.cend(),[&](const TargetPoint_sptr& t_Pose){
...
const TargetPoint_sptr RobotProgram::getWaypoint_byCommand(RobotCommand_sptr t_CommandPtr) const
{
...
void RobotProgram::replacePoseData(const std::vector<TargetPoint_sptr> &t_PoseData)
{
    m_waypoints.clear();
