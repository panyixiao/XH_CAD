...
{
    std::string TracName = getUniqueObjectName("Trajectory");
 
    openCommand("Create trajectory");
    doCommand(Doc,"App.activeDocument().addObject(\"Robot::TrajectoryObject\",\"%s\")",TracName.c_str());
    updateActive();
    doCommand(Gui, "Gui.activeDocument().setEdit('%s')", TracName.c_str());
    commitCommand();
	...
	:Command("Robot_InsertWaypoint")
{
    ...
    sToolTipText    = QT_TR_NOOP("Insert robot tip position into trajectory");
...
{
//    unsigned int n1 = getSelection().countObjectsOfType(Robot::Robot6AxisObject::getClassTypeId());
...
//    Robot::Robot6AxisObject *pcRobotObject=0;
//    if(Sel[0].pObject->getTypeId() == Robot::Robot6AxisObject::getClassTypeId())
//        pcRobotObject = dynamic_cast<Robot::Robot6AxisObject*>(Sel[0].pObject);
//    else if(Sel[1].pObject->getTypeId() == Robot::Robot6AxisObject::getClassTypeId())
//        pcRobotObject = dynamic_cast<Robot::Robot6AxisObject*>(Sel[1].pObject);
//    // 20220302 QuickFix
//    if(pcRobotObject == 0)
//        return;
...
//    // 20220302 QuickFix: Trac Has been Dressedup, it is a trajectoryObject, but the pointer convert failed
//    if(pcTrajectoryObject == 0)
//        return;

//    std::string TrakName = pcTrajectoryObject->getNameInDocument();

//    openCommand("Insert waypoint");
////    doCommand(Doc,"App.activeDocument().%s.Trajectory = App.activeDocument().%s.Trajectory.insertWaypoints(Robot.Waypoint(App.activeDocument().%s.TipPose_inWorld),type='LIN',name='Pt',vel=_DefSpeed,cont=_DefCont,acc=_DefAccelaration,tool=1)",
////              TrakName.c_str(),
////              TrakName.c_str(),
////              RoboName.c_str(),
////              RoboName.c_str());
//    doCommand(Doc,"App.activeDocument().%s.Trajectory = App.activeDocument().%s.Trajectory.insertWaypoints(Robot.Waypoint(App.activeDocument().%s.TipPose_inWorld))",
//              TrakName.c_str(),
//              TrakName.c_str(),
//              RoboName.c_str(),
//              RoboName.c_str());
...
//        if(ObjectFilter.Result.empty())
//            return;
//        if(ObjectFilter.Result.front().empty())
//            return;
