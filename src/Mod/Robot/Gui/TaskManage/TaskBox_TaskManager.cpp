...
bool TaskBox_TaskManager::accept() {
    if(m_Robot != nullptr){
        m_Robot->Activated.setValue(false);
    }
...
void TaskBox_TaskManager::initUi_PanelWidget() {
  ...
  QObject::connect(m_ui->Button_Edit, SIGNAL(clicked()),
                   this, SLOT(slot_editSelectedAction()));
  QObject::connect(m_ui->Button_Delete, SIGNAL(clicked()),
  ...
  QObject::connect(m_ui->pushButton_ExportProgram, SIGNAL(clicked()),
                   this, SLOT(slot_exportProgram()));

  QObject::connect(m_ui->pushButton_readFromFile, SIGNAL(clicked()),
                   this, SLOT(slot_insertAction_ReadinTrac()));
...
void TaskBox_TaskManager::udpateOperatorList()
{
      ...
      auto t_Groups = m_currentDoc->getObjectsOfType(Robot::MechanicGroup::getClassTypeId());
      ...
      for (auto opt_Ptr : t_Groups) {
          m_ui->comboBox_OperatorList->addItem(tr(opt_Ptr->getNameInDocument()));
      }
}

bool TaskBox_TaskManager::slot_insertAction_FreerunTrac()
{
    Q_EMIT Signal_insertFreerunTrac(m_ui->comboBox_OperatorList->currentText());
}

bool TaskBox_TaskManager::slot_insertAction_ReadinTrac()
{
    Q_EMIT Signal_insertTracFromFile(m_ui->comboBox_OperatorList->currentText());
}

bool TaskBox_TaskManager::slot_insertAction_ScanTrac()
{
    Q_EMIT Signal_insertScanTrac(m_ui->comboBox_OperatorList->currentText());
}

bool TaskBox_TaskManager::slot_insertAction_SeamTrac()
{
    Q_EMIT Signal_insertSeamTrac(m_ui->comboBox_OperatorList->currentText());
}
