...
void TaskBox_FreerunTracEditor::slot_changeOperatorTool()
{
    if(m_ui->radioButton_NoTool->isChecked()){
        ...
        if(m_ui->radioButton_useTorch->isChecked())
            ...
            m_MechGroup->setCurrentToolType(Robot::ToolType::Scanner);

        else if(m_ui->radioButton_useCamera->isChecked())
            m_MechGroup->setCurrentToolType(Robot::ToolType::DepthCamera);
    }
}
...
void TaskBox_FreerunTracEditor::updatePoseLabel(const RobotWaypoint_sptr t_wpnt) {
...
bool TaskBox_FreerunTracEditor::slot_insertCommand_Move() {
    ...
    if(t_type == Robot::PoseType::JNTS){
        m_FreeTrac->insertCMD_MOVE(std::string(m_MechGroup->getNameInDocument()),
                                   Robot::RobotWaypoint(m_MechGroup->getCurrentGroupPose(Robot::CordType::ACS)),
                                   Robot::MoveType::MOVJ,
                                   Robot::MovePrec::FINE,
                                   pointSpeed);
    }
        ...
        if(m_ui->radioButton_NoTool->isChecked()){
            m_FreeTrac->insertCMD_MOVE(std::string(m_MechGroup->getNameInDocument()),
                                       Robot::RobotWaypoint(m_MechGroup->getCurrentGroupPose(Robot::CordType::WCS)),
                                       Robot::MoveType::MOVL,
                                       t_Prec,
                                       pointSpeed);
        }
            ...
            m_FreeTrac->insertCMD_MOVE(std::string(m_MechGroup->getNameInDocument()),
                                       Robot::RobotWaypoint(m_MechGroup->getCurrentGroupPose(Robot::CordType::TCS)),
                                       Robot::MoveType::MOVL,
                                       t_Prec,
                                       pointSpeed);
        }
...
void TaskBox_FreerunTracEditor::updateGroupPose(const RobotWaypoint_sptr t_wpntPtr)
{
    if(m_MechGroup != nullptr){
